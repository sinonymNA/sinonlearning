...
export default function ScholarshipResults({
  ...
  // Merge static + scraped scholarship pool
  const allScholarships = useMemo<Scholarship[]>(() => {
    const existingIds = new Set(staticScholarships.map((s) => s.id));
    const uniqueScraped = initialScraped.filter((s) => !existingIds.has(s.id));
    return [...staticScholarships, ...uniqueScraped];
  }, [initialScraped]);
  ...
  const matched = useMemo(() => {
    if (!profile) return [];
    return matchScholarships(profile, allScholarships);
  }, [profile, allScholarships]);
  ...
  if (profile.intendedMajor) summaryParts.push(profile.intendedMajor.replace(/_/g, " "));

  const scrapedCount = initialScraped.length;
    ...
    <div className="min-h-screen bg-cream-50">
      ...
      <div className="sticky top-16 z-30 border-b border-navy-900/8 bg-white/90 backdrop-blur-md">
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-6 py-3">
          ...
          <div className="flex items-center gap-3 text-sm text-navy-800/60">
            <span>
