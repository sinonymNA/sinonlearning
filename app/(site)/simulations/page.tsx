import type { Metadata } from "next";
import { Gamepad2, LineChart, Target } from "lucide-react";
import FadeIn from "@/components/FadeIn";
import SimulationCard from "@/components/simulations/SimulationCard";
import { simulations } from "@/data/simulations";
import { SITE_URL } from "@/lib/seo";

export const metadata: Metadata = {
  title: "Simulations & Games — Sinon Learning",
  description:
    "Interactive simulations that teach real-world economics and finance by letting students make the decisions themselves.",
  alternates: { canonical: `${SITE_URL}/simulations` },
};

const pillars = [
  {
    icon: Target,
    title: "Make real decisions",
    description: "No multiple choice—students set prices, budgets, and strategies themselves.",
  },
  {
    icon: LineChart,
    title: "See the consequences",
    description: "Every choice plays out visually, so cause and effect actually click.",
  },
  {
    icon: Gamepad2,
    title: "Learn by replaying",
    description: "Short, replayable rounds make it easy to test a new strategy and improve.",
  },
];

export default function SimulationsPage() {
  return (
    <div className="bg-navy-950">
      <section className="bg-circuit relative overflow-hidden px-6 pt-16 pb-20 lg:px-8 lg:pt-24">
        <div className="absolute left-1/2 top-0 -z-10 h-[520px] w-[520px] -translate-x-1/2 -translate-y-1/3 rounded-full bg-teal-400/15 blur-[130px]" />
        <div className="absolute -right-24 top-32 -z-10 h-80 w-80 rounded-full bg-purple-500/15 blur-[120px]" />
        <div className="absolute -left-24 bottom-0 -z-10 h-72 w-72 rounded-full bg-fuchsia-500/10 blur-[110px]" />

        <div className="mx-auto max-w-5xl text-center">
          <FadeIn>
            <span className="inline-flex items-center gap-2 rounded-full border border-teal-300/25 bg-teal-300/5 px-4 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-teal-200">
              Mission Three
            </span>
          </FadeIn>
          <FadeIn delay={0.05}>
            <h1 className="mt-6 font-display text-4xl font-medium leading-tight text-white sm:text-6xl">
              Simulations &amp; Games
            </h1>
          </FadeIn>
          <FadeIn delay={0.1}>
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-relaxed text-white/65">
              Sinon Learning&rsquo;s third mission: interactive simulations that turn abstract
              ideas in economics and personal finance into decisions students can actually make.
            </p>
          </FadeIn>
        </div>

        <FadeIn delay={0.15}>
          <div className="mx-auto mt-14 grid max-w-4xl grid-cols-1 gap-4 sm:grid-cols-3">
            {pillars.map((pillar) => (
              <div
                key={pillar.title}
                className="rounded-2xl border border-white/10 bg-white/[0.03] p-5 text-center"
              >
                <span className="mx-auto flex h-10 w-10 items-center justify-center rounded-full border border-teal-300/20 bg-teal-300/10 text-teal-200">
                  <pillar.icon size={18} />
                </span>
                <p className="mt-3 text-sm font-semibold text-white">{pillar.title}</p>
                <p className="mt-1.5 text-xs leading-relaxed text-white/50">{pillar.description}</p>
              </div>
            ))}
          </div>
        </FadeIn>
      </section>

      <section className="px-6 py-20 lg:px-8">
        <div className="mx-auto max-w-7xl">
          <FadeIn>
            <div className="mb-10 text-center">
              <p className="text-sm font-semibold uppercase tracking-[0.14em] text-teal-300/80">
                Flagship Simulation
              </p>
              <h2 className="mt-3 font-display text-3xl font-medium text-white sm:text-4xl">
                Start with the lemonade stand.
              </h2>
            </div>
          </FadeIn>
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
            {simulations.map((simulation, i) => (
              <FadeIn key={simulation.slug} delay={i * 0.08}>
                <SimulationCard simulation={simulation} />
              </FadeIn>
            ))}
          </div>
        </div>
      </section>
    </div>
  );
}
