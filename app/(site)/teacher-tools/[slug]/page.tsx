...
import { getTeacherToolBySlug } from "@/data/teacherTools";
...
export async function generateMetadata({
  ...
  const entry = getTeacherToolBySlug(slug);
  if (!entry) return {};

  return {
    title: `${entry.title} — Sinon Learning`,
    description: entry.description,
    alternates: { canonical: `${SITE_URL}/teacher-tools/${entry.slug}` },
  };
}

export default async function TeacherToolPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
