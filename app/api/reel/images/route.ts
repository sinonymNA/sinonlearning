...
import { createReelImage } from "@/lib/reelDb";
...
// Same SSRF guard used by Slider's image route: resolve the hostname up front
// and reject localhost/private/link-local targets before fetching a
// teacher-supplied image URL.
...
export async function POST(request: NextRequest) {
  ...
  if (isRateLimited(`reel-images:${ip}`, RATE_LIMIT_WINDOW_MS, RATE_LIMIT_MAX)) {
  ...
  if (contentType.includes("multipart/form-data")) {
    ...
    const image = await createReelImage({ uploadedBy: user.id, mimeType: file.type, data: buffer });
  ...
  const image = await createReelImage({
    uploadedBy: user.id,
