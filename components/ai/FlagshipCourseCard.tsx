...
export default function FlagshipCourseCard({ course }: { course: AIFlagshipCourse }) {
    ...
    >
      ...
      <div className="mt-6 flex items-center justify-between border-t border-white/10 pt-5">
        <span className={`text-xs font-medium tracking-wide ${available ? "text-white/50" : "text-white/30"}`}>
          {course.duration} · {course.gradeBand}
        </span>
        {available && (
  ...
  return (
    <Link href={`/ai/${course.slug}`} className="block h-full">
      {inner}
    </Link>
  );
}
