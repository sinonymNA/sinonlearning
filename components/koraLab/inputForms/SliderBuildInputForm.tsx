...
export interface SliderBuildInput {
  topic: string;
  audience: string;
  keyPoints: string;
  length: string;
...
export function defaultSliderBuildInput(): SliderBuildInput {
  return { topic: "", audience: "", keyPoints: "", length: "Medium (~8 slides)", notes: "" };
}

export default function SliderBuildInputForm({
  value,
  onChange,
}: {
  value: SliderBuildInput;
  onChange: (v: SliderBuildInput) => void;
}) {
  return (
    <div className="flex flex-col gap-3">
      ...
      <label className="flex flex-col gap-1.5">
        ...
        <select
          ...
          className="rounded-lg border border-navy-900/12 bg-white px-3 py-2 text-sm text-navy-900 outline-none focus:border-teal-500/50"
        >
          <option>Short (~5 slides)</option>
          <option>Medium (~8 slides)</option>
          <option>Long (~12 slides)</option>
        </select>
      </label>
      <label className="flex flex-col gap-1.5">
