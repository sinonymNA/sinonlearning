...
import { WRITING_MECHANICS_SKILL_IDS, WRITING_MECHANICS_LABELS } from "@/lib/marginsPracticeCourses";
import type { MasteryLevel } from "@/lib/marginsDb";

interface WritingMechanicsEntry {
  skill: string;
  level: MasteryLevel;
}

interface Props {
  mastery: WritingMechanicsEntry[];
  title?: string;
}

// Sibling to SkillMasteryPanel, same shape and level styling, but iterating
// the course-specific claim/evidence/reasoning/identify-vs-explain dimension
// instead of the six official AP skills — deliberately never merged into
// SkillMasteryPanel itself.
...
export default function WritingMechanicsPanel({ mastery, title = "SAQ writing mechanics" }: Props) {
  const containerRef = useRef<HTMLDivElement>(null);
  useMountReveal(containerRef, ".mechanics-row", { stagger: 80, translateY: 12, duration: 380 });
    ...
    <div ref={containerRef} className="rounded-2xl border border-teal-100 bg-white p-5">
      ...
      <div className="flex flex-col gap-3.5">
        {WRITING_MECHANICS_SKILL_IDS.map((skill) => {
          ...
          return (
            <div key={skill} className="mechanics-row" style={{ opacity: 0 }}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-[13px] font-semibold text-stone-700">{WRITING_MECHANICS_LABELS[skill]}</span>
                <span
