...
export default function TakeAStarScene({ objective, quality, reducedMotion, pointerLock, onNearChange, onPosition }: Props) {
  ...
  useEffect(() => {
    ...
    const deck = new THREE.Mesh(new THREE.BoxGeometry(9.2, .35, 18), wood); deck.position.y = .1; deck.receiveShadow = true; world.add(deck);
    const bow = new THREE.Mesh(new THREE.ConeGeometry(4.6, 5.5, 4), wood); bow.rotation.x = Math.PI / 2; bow.rotation.y = Math.PI / 4; bow.position.set(0, .1, -11); world.add(bow);
    [-4.5, 4.5].forEach((x) => { const rail = new THREE.Mesh(new THREE.BoxGeometry(.22, 1.2, 17.5), darkWood); rail.position.set(x, .8, 0); rail.castShadow = true; world.add(rail); });
    for (let z = -7; z <= 7; z += 2) for (const x of [-4.25, 4.25]) { const post = new THREE.Mesh(new THREE.CylinderGeometry(.055, .065, 1.55, 8), darkWood); post.position.set(x, 1.35, z); world.add(post); }
    const mast = new THREE.Mesh(new THREE.CylinderGeometry(.22, .3, 13, 14), darkWood); mast.position.set(0, 6.4, -.7); mast.castShadow = true; world.add(mast);
    const yard = new THREE.Mesh(new THREE.CylinderGeometry(.1, .13, 9, 10), darkWood); yard.rotation.z = Math.PI / 2; yard.position.set(0, 8.4, -.7); world.add(yard);
    const sail = new THREE.Mesh(new THREE.PlaneGeometry(7.8, 5.2, 7, 5), cloth); sail.position.set(0, 5.7, -.75); sail.rotation.y = .03; sail.castShadow = true; world.add(sail);
    addRope(world, new THREE.Vector3(0, 12.8, -.7), new THREE.Vector3(-4.2, 1.4, 7.5));
    addRope(world, new THREE.Vector3(0, 12.8, -.7), new THREE.Vector3(4.2, 1.4, 7.5));
    addRope(world, new THREE.Vector3(-4.4, 8.4, -.7), new THREE.Vector3(-4.2, 1.4, -6.5));
    addRope(world, new THREE.Vector3(4.4, 8.4, -.7), new THREE.Vector3(4.2, 1.4, -6.5));
    ...
    const keys = new Set<string>(); let yaw = 0; let pitch = -.05; let dragging = false; let lastX = 0; let lastY = 0; let near: "navigator" | "astrolabe" | "observation" | null = null;
    const onKeyDown = (e: KeyboardEvent) => keys.add(e.code); const onKeyUp = (e: KeyboardEvent) => keys.delete(e.code);
    const onMouseMove = (e: MouseEvent) => { if (document.pointerLockElement === renderer.domElement || dragging) { const dx = document.pointerLockElement ? e.movementX : e.clientX - lastX; const dy = document.pointerLockElement ? e.movementY : e.clientY - lastY; yaw -= dx * .0022; pitch = THREE.MathUtils.clamp(pitch - dy * .002, -.72, .62); lastX = e.clientX; lastY = e.clientY; } };
    ...
    const animate = () => {
      ...
      const forward = new THREE.Vector3(-Math.sin(yaw), 0, -Math.cos(yaw)); const right = new THREE.Vector3(Math.cos(yaw), 0, -Math.sin(yaw)); const movement = new THREE.Vector3();
      if (keys.has("KeyW") || keys.has("ArrowUp")) movement.add(forward); if (keys.has("KeyS") || keys.has("ArrowDown")) movement.sub(forward); if (keys.has("KeyD") || keys.has("ArrowRight")) movement.add(right); if (keys.has("KeyA") || keys.has("ArrowLeft")) movement.sub(right);
      if (movement.lengthSq()) { movement.normalize().multiplyScalar(dt * 3.1); camera.position.add(movement); }
      camera.position.x = THREE.MathUtils.clamp(camera.position.x, -3.75, 3.75); camera.position.z = THREE.MathUtils.clamp(camera.position.z, -8.2, 8.2); camera.position.y = 2.05 + (reducedMotion ? 0 : Math.sin(t * .8) * .025);
      camera.rotation.order = "YXZ"; camera.rotation.y = yaw; camera.rotation.x = pitch + (reducedMotion ? 0 : Math.sin(t * .55) * .006); camera.rotation.z = reducedMotion ? 0 : Math.sin(t * .48) * .008;
