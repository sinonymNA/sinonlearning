...
interface StudioTopBarProps {
  ...
  onOpenExport: () => void;
  onDuplicate: () => void;
  /** When set, the back control runs this instead of linking to /studio (used to return to the project hub). */
  onBack?: () => void;
  backLabel?: string;
  /** Hides the subject/grade/duration inputs — used inside the focused editors where that detail lives on the hub. */
  compact?: boolean;
  /** A short context label shown next to the title (e.g. "Slides", "Worksheet"). */
  contextLabel?: string;
}
...
export default function StudioTopBar({
  ...
  onDuplicate,
  onBack,
  backLabel = "Back to Teacher Studio",
  compact = false,
  contextLabel,
}: StudioTopBarProps) {
    ...
    <div className="no-print flex flex-wrap items-center gap-3 border-b border-navy-900/8 bg-white px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          {contextLabel && (
            <span className="flex-shrink-0 text-sm font-medium text-navy-700/40">{contextLabel} ·</span>
          )}
          <input
            value={project.title}
