...
import {
  ArrowLeft, Check, ChevronDown, Flame, Gem, Heart, RotateCcw, X, Zap,
} from "lucide-react";
...
type Phase = "home" | "room" | "shrine" | "lost" | "extracted";
...
function playTone(type: "correct" | "wrong" | "tick" | "fanfare") {
  try {
    ...
    if (type === "correct") { schedule(440, 0, 0.12, "sine", 0.22); schedule(660, 0.1, 0.25, "sine", 0.2); }
    else if (type === "wrong") { schedule(200, 0, 0.15, "sawtooth", 0.18); schedule(130, 0.15, 0.35, "sawtooth", 0.15); }
    else if (type === "tick") { schedule(900, 0, 0.05, "square", 0.07); }
    else if (type === "fanfare") { [330, 415, 523, 660].forEach((f, i) => schedule(f, i * 0.1, 0.2, "sine", 0.15)); }
...
export default function VaultGame() {
  ...
  const [relicToast, setRelicToast] = useState<VaultArtifact | null>(null);
  ...
  // ── Countdown timer ─────────────────────────────────────────────────────────
  // Deps include picked/timedOut so cleanup fires immediately when user answers,
  // preventing the interval from interfering with the auto-advance timeout.
    ...
    const id = setInterval(() => {
      setTimer(t => {
        if (t <= 1) {
          ...
          setTimedOut(true);
          setCombo(0);
          const newLight = Math.max(0, lightRef.current - (modRef.current === "fortune" ? 2 : 1));
          setLight(newLight);
          playTone("wrong");
          setTimeout(() => {
            if (newLight <= 0) { setPhase("lost"); return; }
            goNext(depth, newLight);
          }, 1900);
          return 0;
        }
  ...
  /** Advance to next floor, or extract if at the end. Shrine at floors 3→4 and 6→7. */
  const goNext = (fromDepth: number, currentLight?: number) => {
    const lives = currentLight ?? light;
    if (lives <= 0) { setPhase("lost"); return; }
    if (fromDepth >= 8) { doExtract(); return; }
  ...
  const start = () => {
    ...
    setRelicToast(null); setCarried([]); setModifier(null);
    setRunWins({});
    resetRoom();
    setPhase("room");
  };
  ...
  const answer = (choiceIndex: number) => {
    ...
    if (!isCorrect) {
      playTone("wrong");
      setCombo(0);
      const newLight = Math.max(0, light - (modifier === "fortune" ? 2 : 1));
      setLight(newLight);
      // Show the revealed correct answer for 1.9s, then auto-advance
      setTimeout(() => goNext(depth, newLight), 1900);
      return;
    }
    ...
    if (nextCombo > 0 && nextCombo % 3 === 0) {
      import("canvas-confetti").then(mod => {
        mod.default({ particleCount: 55, spread: 70, origin: { y: 0.55 }, colors: ["#fbbf24", "#34d399", "#60a5fa", "#f472b6"] });
      });
    }

    // Even floors: show relic as a brief toast, then advance
    if (depth % 2 === 0) {
      playTone("fanfare");
      ...
      setRelicToast(found);
      setTimeout(() => goNext(depth), 2400);
    } else {
      setTimeout(() => goNext(depth), 1600);
    }
  };
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0f] text-white">
      <header className="sticky top-0 z-30 flex h-14 items-center justify-between border-b border-white/8 bg-[#0a0a0f]/90 px-4 backdrop-blur-xl sm:px-6">
        ...
        <span className="text-2xl tracking-[0.12em] text-white" style={{ fontFamily: "var(--font-bebas)" }}>
          THE VAULT
        </span>
      ...
      <AnimatePresence mode="wait">
        ...
        {phase !== "home" && (
          ...
          >
            ...
            {phase === "lost" && (
              <EndScreen lost shards={Math.floor(shards / 2)} depth={depth} onAgain={start} onHome={() => setPhase("home")} />
            )}

            {phase === "extracted" && (
              <EndScreen shards={shards} depth={depth} onAgain={start} onHome={() => setPhase("home")} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
    ...
    <div className="mb-6 flex items-center justify-between border-b border-white/8 pb-4">
      <div className="flex items-center gap-1">
        {Array.from({ length: 3 }).map((_, i) => (
          <Heart key={i} size={16} className={i < light ? "fill-red-400 text-red-400" : "text-white/15"} />
        ))}
      </div>
      ...
      <div className="flex items-center gap-3">
        {combo >= 2 && (
          <motion.span key={combo} initial={{ scale: 1.4 }} animate={{ scale: 1 }} className="flex items-center gap-1 text-xs font-black text-orange-300">
            <Flame size={13} /> {combo}×
          </motion.span>
        )}
        <span className="flex items-center gap-1.5 text-xs font-bold text-yellow-300">
...
function RoomScreen({ question, order, picked, timedOut, eliminated, modifier, timerPct, timerColor, timer, relicToast, onEcho, onAnswer }: {
        ...
        {order.map((choiceIndex, pos) => {
          ...
          return (
            <motion.button
              ...
              className="relative flex min-h-[90px] flex-col items-start gap-2 rounded-2xl border-2 p-4 text-left text-white sm:min-h-[100px]"
            >
              ...
              {isAnswered && isCorrect && <Check size={16} className="absolute right-3 top-3" />}
              {isAnswered && isPicked && !isCorrect && <X size={16} className="absolute right-3 top-3 text-red-300" />}
            </motion.button>
          );
        })}
      </div>

      {/* Feedback row: time-up or explanation */}
      <AnimatePresence>
        {timedOut && (
          <motion.p key="timeout" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="mt-4 text-sm font-bold text-red-400">
            Time&apos;s up — correct answer:{" "}
            <span className="font-normal text-white">{question.choices[question.answer]}</span>
          </motion.p>
        )}
        {picked !== null && !timedOut && (
          <motion.p key="explain" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="mt-5 border-l-2 border-cyan-500/40 pl-4 text-sm leading-relaxed text-slate-400">
            {question.explanation}
          </motion.p>
        )}
      </AnimatePresence>

      {/* Relic toast — slides up briefly, then game auto-advances */}
      <AnimatePresence>
        {relicToast && (
          <motion.div
            key="relic"
            ...
            className="mt-6 flex items-center gap-4 rounded-2xl border border-yellow-300/25 bg-yellow-300/10 px-5 py-4"
          >
            <span className="text-4xl">{relicToast.glyph}</span>
            <div>
              <p className="text-[10px] font-black uppercase tracking-widest text-yellow-400">Relic Found</p>
              <p className="mt-0.5 font-bold text-white" style={{ fontFamily: "var(--font-bebas)", fontSize: "1.25rem", letterSpacing: "0.06em" }}>
                {relicToast.name}
              </p>
              <p className="mt-0.5 text-xs italic text-slate-500">{relicToast.lore}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
...
// ─── Shrine ───────────────────────────────────────────────────────────────────
// One-click power-up select — no two-phase confirmation.

const SHRINE_BOONS: Array<{ id: Exclude<Modifier, null>; name: string; icon: string; desc: string; accent: string }> = [
  { id: "echo",    name: "Echo Lens",    icon: "◉", desc: "Remove a wrong answer",     accent: "#0891b2" },
  { id: "fortune", name: "Fortune Oath", icon: "⚡", desc: "+50% shards · 2 lives lost", accent: "#b45309" },
  { id: "lantern", name: "Deep Lantern", icon: "🔦", desc: "Restore one life",            accent: "#7c3aed" },
];

function ShrineScreen({ depth, shards, active, onBoon, onExtract }: {
  depth: number;
  shards: number;
  active: Modifier;
  onBoon: (mod: Exclude<Modifier, null>) => void;
  onExtract: () => void;
}) {
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center text-center">
      <p className="mb-1 text-[10px] font-black uppercase tracking-[0.28em] text-violet-400">
        Floor {depth + 1} unlocked
      </p>
      <h2 className="mb-8 text-6xl text-white" style={{ fontFamily: "var(--font-bebas)", letterSpacing: "0.06em" }}>
        PICK A POWER
      </h2>

      <div className="flex w-full max-w-sm flex-col gap-3">
        {SHRINE_BOONS.map(boon => (
          <motion.button
            key={boon.id}
            onClick={() => onBoon(boon.id)}
            whileHover={{ scale: 1.02, y: -2 }}
            whileTap={{ scale: 0.97 }}
            style={{ background: boon.accent, borderColor: active === boon.id ? "#fff" : "transparent" }}
            className="flex items-center gap-4 rounded-2xl border-2 px-5 py-4 text-left text-white transition-opacity"
          >
            <span className="text-3xl w-9 text-center">{boon.icon}</span>
            <div>
              <div className="font-black tracking-wide" style={{ fontFamily: "var(--font-bebas)", fontSize: "1.15rem", letterSpacing: "0.08em" }}>
                {boon.name}
                {active === boon.id && <span className="ml-2 text-xs font-bold normal-case tracking-normal opacity-60">active</span>}
              </div>
              <div className="mt-0.5 text-xs text-white/70">{boon.desc}</div>
            </div>
          </motion.button>
        ))}
        ...
        <button
          onClick={onExtract}
          className="mt-1 rounded-xl border border-yellow-400/30 bg-yellow-400/10 px-6 py-3.5 text-sm font-bold text-yellow-200 transition-colors hover:bg-yellow-400/20"
        >
          Extract now · {shards} shards
        </button>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex min-h-[55vh] flex-col items-center justify-center text-center">
      <motion.div
        ...
        className={`mb-6 flex h-20 w-20 items-center justify-center rounded-2xl border ${lost ? "border-red-400/30 bg-red-500/15" : "border-cyan-400/30 bg-cyan-400/15"}`}
      >
        {lost ? <Flame size={36} className="text-red-300" /> : <Gem size={36} className="text-cyan-300" />}
      </motion.div>
      ...
      <h2 className="mb-4 text-6xl text-white" style={{ fontFamily: "var(--font-bebas)", letterSpacing: "0.06em" }}>
        {lost ? "VAULT WINS." : "YOU MADE IT."}
      </h2>
      <p className="mb-8 text-slate-400">
        <span className="text-xl font-black text-yellow-300">{shards}</span> shards recovered.
      </p>

      <div className="flex gap-3">
        <button onClick={onHome} className="rounded-xl border border-white/15 px-5 py-3 text-xs font-bold uppercase tracking-wider text-slate-300 transition-colors hover:bg-white/5">
          Home
        </button>
        <button onClick={onAgain} className="flex items-center gap-2 rounded-xl bg-white px-5 py-3 text-xs font-black uppercase tracking-wider text-black transition-colors hover:bg-white/90">
  ...
  return (
    <section className="relative flex min-h-[calc(100vh-56px)] flex-col items-center justify-center px-6 py-16 text-center">
      ...
      <motion.div initial={{ opacity: 0, y: -16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="relative">
        <p className="mb-5 text-[10px] font-black uppercase tracking-[0.35em] text-cyan-400">Knowledge Descent</p>
        <h1 className="text-[clamp(5rem,20vw,11rem)] leading-none text-white" style={{ fontFamily: "var(--font-bebas)" }}>
          THE<br /><span className="text-yellow-300">VAULT</span>
        </h1>
        <p className="mx-auto mt-6 max-w-xs text-sm leading-relaxed text-slate-400">
          8 floors. 3 lives. 12 seconds per question.<br />Answer fast, build a combo, get out richer.
        </p>
      </motion.div>
      ...
      >
        ...
        <Link href="/vault/build" className="text-xs text-slate-600 transition-colors hover:text-slate-300">
          Build a custom question set →
        </Link>
      </motion.div>

      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }} className="relative mt-12 flex gap-10">
        {[
          ...
          <div key={label} className="text-center">
            ...
            <div className="mt-1 text-[10px] font-medium uppercase tracking-wider text-slate-600">{label}</div>
          </div>
        ))}
      </motion.div>
