...
function buildNotesheetUserMessage(params: NotesheetGenerateInput & { targetPages: number }): string {
  ...
  return [
    ...
    `17. exit_ticket: Use ONLY as the FINAL section on the notesheet for end-of-class formative assessment. student_prompt = 2-3 complete questions separated by newline (\\n), each directly targeting the lesson's learning objective. num_lines = number of questions. Example: "What is the difference between a movement along the demand curve and a shift of the curve?\\nName two factors that shift supply to the right." Never place exit_ticket anywhere but last.`,
