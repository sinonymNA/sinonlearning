...
function buildNotesheetUserMessage(params: NotesheetGenerateInput & { targetPages: number }): string {
  ...
  return [
    ...
    `Space budget per section type (approximate): warmup_box=small, fill_blank=small, numbered_response=medium, two_column_box=large, three_column_box=large, drawing_box=large, content_box=small, structured_concept_box=large, graph_box=large, acronym_scaffold=medium, labeled_comparison_table=large.`,
    `For a ${targetPages}-page sheet, use at most ${targetPages <= 2 ? "1 large section (two_column_box, three_column_box, drawing_box, structured_concept_box, graph_box, or labeled_comparison_table)" : "2 large sections"}. Prefer warmup_box, fill_blank, and numbered_response for the rest.`,
